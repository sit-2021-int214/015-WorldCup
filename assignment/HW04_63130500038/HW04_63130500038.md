...
# Clean Dataset
superstore_sales <- clean_names(superstore_sales )
colnames(superstore_sales )
```
...
superstore_sales  %>% 
  ...
  filter((superstore_sales $product_name %like% 'Bush'),(superstore_sales$state == "New York"))
```
Result:
```
...
2.)
```
superstore_sales$customer_name <- as.factor(superstore_sales$customer_name)
summary(superstore_sales$customer_name)
